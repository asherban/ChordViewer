...
`src/lead-sheet-v1.schema.json` and `src/lead-sheet-v2.schema.json` define field shapes; runtime validation also checks IDs, event order, overlap, bar boundaries and ties. The TypeScript validator lives in `src/validation.ts`; native decoding mirrors it in `LeadSheetReader`. Shared fixtures under `tests/fixtures/music` verify the contract on both platforms.

`src/score.ts` contains score types, timing and pitch-comparison primitives. Validation and authoring modules import their dependencies directly; `src/index.ts` only reexports the public package API. Keep internal imports out of that public barrel so adding a public export cannot create a runtime dependency cycle. `src/library.ts` defines the shared account and sheet request/response types.

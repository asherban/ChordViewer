...
data class MeasureTimeline(val ticks: List<Int>, val positions: List<Float>) {
    ...
    fun x(tick: Int, available: Float): Float {
        require(tick in 0..ticks.last() && available >= width)
        ...
        return natural + (available - width) * tick / ticks.last()
    }
}
...
object ScoreLayout {
    fun geometry(sheet: LeadSheet): StaffGeometry {
        ...
        val top = maxOf(keyTop, 26f + (steps.maxOrNull() ?: 8) * 5)
    ...
    fun timeline(measure: ScoreMeasure, melody: Boolean, symbolWidth: (String) -> Float, barTicks: Int, keySignature: String): MeasureTimeline {
        ...
        if (melody) {
            ...
            val notation = marks(LeadSheet("layout", "Layout", listOf(measure), keySignature = keySignature)).first()
        ...
        gaps.forEachIndexed { index, gap -> positions += positions.last() + gap + spare * (ticks[index + 1] - ticks[index]) / barTicks }
    ...
    fun systems(sheet: LeadSheet, timelines: List<MeasureTimeline>, available: Float, melody: Boolean): List<ScoreSystem> {
        ...
        while (first < sheet.measures.size) {
            val fifths = kotlin.math.abs(KEY_SIGNATURES.getValue(sheet.keySignature).fifths)
            val keyWidth = fifths * 12 + if (fifths > 0) 6 else 0
            val prefix = if (!melody) 0f else if (first == 0) 62f + keyWidth + (if (sheet.timeSignature.numerator >= 10) 16 else 0) else 36f + keyWidth
    ...
    fun marks(sheet: LeadSheet): List<List<NotationMark>> {
        val signature = keyAccidentals(sheet.keySignature)
        ...
        return sheet.measures.map { measure ->
            ...
            measure.melody.map { event ->
                ...
                val accidental = pitch?.let {
                    ...
                    val previousAlter = alterations[key] ?: signature.getValue(it.step)

...
export function chordSegments(measure: Measure, textWidth: (text: string) => number) {
  const boundaries = [...new Set([0, 1920, ...measure.chords.flatMap(chord => [chord.offsetTicks, chord.offsetTicks + chord.durationTicks])])].sort((a, b) => a - b);
...
export function positionChordSegments(segments: ReturnType<typeof chordSegments>, width: number) {
  ...
  return segments.map(segment => {
    const span = segment.minimum + elastic * (segment.end - segment.start) / 1920;

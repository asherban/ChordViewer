...
Planning baseline: 2026-09-20. This is an outcome-level roadmap based on the [product plan](product-plan.md) and [setup and cleanup plan](setup-and-cleanup.md). It does not assign dates or low-level tasks. Only M0 is complete; all implementation milestones are planned.
...
Routine acceptance uses the local browser and Android emulator with loopback MIDI. A piano and tablet are not required. See [Local MIDI testing](local-midi-testing.md) for the successful web smoke test and the Android bridge still to build; this partial evidence does not complete M1.

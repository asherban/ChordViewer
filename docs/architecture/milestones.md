...
Planning baseline: 2026-09-20. This is an outcome-level roadmap based on the [product plan](product-plan.md) and [setup and cleanup plan](setup-and-cleanup.md). It does not assign dates or low-level tasks. M0 and M1 are complete. See the [M1 verification record](../development/m1-verification.md) for builds, reviews, real MIDI input and native emulator evidence. M2 onward remain planned.

...
Planning baseline: 2026-09-20. This is an outcome-level roadmap based on the [product plan](product-plan.md) and [setup and cleanup plan](setup-and-cleanup.md). It does not assign dates or low-level tasks. **M0–M2 are complete.** See the [M1 verification record](../development/m1-verification.md) for tooling and the [M2 verification record](../development/m2-verification.md) for the rebuilt foundation, notation proof and real MIDI acceptance. M3 onward remain planned.

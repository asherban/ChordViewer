...
This is the full planned coverage, not a claim that every row passes. M4 verifies chord gestures, selected durations, correction/undo/redo, saved-score persistence and interruption recovery. M5 verifies melody input and editing, dots/rests/ties, keys/meters, separate pass preservation and import/export. M6 verifies Manual Practice navigation, chord comparison, fresh-gesture On match advancement, display-only transposition, and no score mutation. Durable offline recovery remains later work. Unit checks replay fixtures directly into each client's processing layer without requiring the Windows driver; opt-in integration tests additionally exercise LoopBe and the emulator bridge.

...
Library holds each account's saved sheets. Create currently supports creating a blank/example sheet and saving its title/tutorial. Practice currently shows the score and live MIDI feedback. Mode changes preserve the open sheet, its unsaved details and the MIDI connection. Explicit sign-out clears the account workspace and MIDI input; backgrounding the app/page disconnects input so stale held notes cannot remain active.

The tutorial panel represents the linked video without a simulated video player. Web can open the canonical YouTube link in another tab; embedded playback comes later. No invented score thumbnails, search results, imports or practice transport controls are presented as working features. MIDI insertion and corrections belong to M4, melody editing/import to M5, and full Library/Practice behavior to M6.

Current implementation screenshots and completed checks are recorded in [UI verification](../development/m3-ui-verification.md). The mockups remain the design reference as future controls become functional.
